...
export class PageLoadHttpInterceptor implements HttpInterceptor {
  ...
  public intercept(
    ...
    if (this.defaultConfig.excludeRequest) {
      if (
        this.defaultConfig.excludeRequest.findIndex((url) =>
          request.url.toLowerCase().endsWith(url)
        ) > -1
      ) {
    ...
    if (this.defaultConfig.excludeRoute) {
      if (
        this.defaultConfig.excludeRoute.findIndex((url) =>
          this.router.url.toLowerCase().endsWith(url)
        ) > -1
      ) {
        return next.handle(request);
      }
